	...
	def findDefSite(defSite : Context) : Node = {
	  ...
	  val icfgN = {
	    if(this.icfg.icfgNormalNodeExists(defSite)) this.icfg.getICFGNormalNode(defSite)
		  ...
		  else if(defSite.toString.startsWith("(EntryPoint,L0000)")) this.icfg.entryNode // else if(defSite.toString == "(EntryPoint,L0000)") this.icfg.entryNode
		  else if(defSite.toString == "(Center,L0000)()") this.icfg.centerNode // else if(defSite.toString == "(Center,L0000)") this.icfg.centerNode
